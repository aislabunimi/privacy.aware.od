...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                """
                tensor([[  6.2939, 239.0686, 485.1873, 922.0530],
                [447.5117, 675.0714, 625.0000, 934.3750],
                [412.6025, 398.9489, 547.3215, 644.6166]], device='cuda:0')
                """
                #print(gt_labels_in_image) #tensor([1, 1, 1], device='cuda:0')
                #print(proposals_in_image)
                """
                tensor([[  0.0000, 145.8887, 765.6924, 900.0000],
                [231.3731, 238.9420, 698.8044, 651.8804],
                [330.7417, 419.6823, 791.9880, 821.9900],
                [  6.2939, 239.0686, 485.1873, 922.0530],
                [447.5117, 675.0714, 625.0000, 934.3750],
                [412.6025, 398.9489, 547.3215, 644.6166]], device='cuda:0')
                """
                ...
                # In rpn.py: get the targets corresponding GT for each proposal. Essenzialmente è la stessa cosa
                ...
                #le ultime 3 proposals sono in realtà GT box in sè, quindi avranno come valore necessariamente 0, 1, 2
                #estraggo gli id
        ...
        #print(matched_idxs) #tensor([0, 0, 0, 0, 1, 2], device='cuda:0')
        #print(labels) #tensor([1, 0, 0, 1, 1, 1], device='cuda:0')] #la 1 e 2 sono 0 perché hanno IoU sotto la soglia
    ...
    def forward(
        ...
        #PASSO 1: uso le proposals della RPN e seleziono quelle che sono interessanti, facendo ROI pooling e cropping (selezionando gli oggetti).
        #lo posso fare vedendo quanto sono buone le proposals rispetto le GT, quanto overlappano, distinguendo
        #le background dalle foreground usando le threshold fg e bg. è MultiScaleRoIAlign
        ...
        #TUTTA QUESTA PARTE SOTTO A NOI NON INTERESSA (Mask, Keypoint ...)
        if self.has_mask():
