def extract_values(lines, split_line_value):
...
def extract_iou50_ap(input_file, output_file, standard_ap=True):
    ...
    while i < len(lines):
        if lines[i].startswith("AP for Epoch"):
            epoch, ap_value, ar_value = extract_values(lines[i:i+14], split_line_value)  # 14 linee per ogni epoca
