...
python3 main.py --use_dataset_subset $USE_DATASET_SUBSET --batch_size_unet $BATCH_SIZE_UNET --save_disturbed_dataset --unet_fw_weights_load ${EXPERIMENT_DIR}/allprop/forward/model_fw_50.pt
...
mv $UNET_WEIGHTS_BW_TO_SAVE1 "${EXPERIMENT_DIR}/allprop/backward_mia"
mv $UNET_WEIGHTS_BW_TO_SAVE2 "${EXPERIMENT_DIR}/allprop/backward_mia"
#rm -rf $RESULTS_DIR "${EXPERIMENT_DIR}/all_proposals/backward/${RESULTS_DIR}_bw"
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/backward_mia/${RESULTS_DIR}_bw"
...
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/test_results_mia"
