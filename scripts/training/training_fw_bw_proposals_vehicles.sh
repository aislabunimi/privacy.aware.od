...
EXPERIMENT_DIR='experiments_5classes'
...
python3 main.py --five_classes --use_dataset_subset $USE_DATASET_SUBSET --batch_size_unet $BATCH_SIZE_UNET --lr_unet $LR_UNET --num_epochs_unet_forward $NUM_EPOCHS_UNET_FORWARD --unet_save_path $UNET_SAVE_PATH --results_dir $RESULTS_DIR --tasknet_weights_load $TASKNET_WEIGHTS_LOAD --not_use_custom_filter_prop
mkdir -p "${EXPERIMENT_DIR}/allprop/forward"
cp $UNET_WEIGHTS_FW_TO_SAVE1 "${EXPERIMENT_DIR}/allprop/forward"
mv $UNET_WEIGHTS_FW_TO_SAVE2 "${EXPERIMENT_DIR}/allprop/forward"
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/forward/${RESULTS_DIR}_fw"
...
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/forward/val_results_batch1"
...
mkdir -p "${EXPERIMENT_DIR}/allprop/backward"
mv $UNET_WEIGHTS_BW_TO_SAVE1 "${EXPERIMENT_DIR}/allprop/backward"
mv $UNET_WEIGHTS_BW_TO_SAVE2 "${EXPERIMENT_DIR}/allprop/backward"
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/backward/${RESULTS_DIR}_bw"
echo "Completed all_proposals backward experiment and copied results to ${EXPERIMENT_DIR} folder"

python3 main.py --five_classes --test_model --tasknet_weights_load $TASKNET_WEIGHTS_LOAD --unet_fw_weights_load "${EXPERIMENT_DIR}/all_proposals/forward/${UNET_FW}" --unet_bw_weights_load "${EXPERIMENT_DIR}/all_proposals/backward/${UNET_BW}" --results_dir $RESULTS_DIR
mv $RESULTS_DIR "${EXPERIMENT_DIR}/allprop/test_results"
...
for i in "3 3" "2 2"; do
   ...
   python3 main.py --five_classes --test_model --tasknet_weights_load $TASKNET_WEIGHTS_LOAD --unet_fw_weights_load "${EXPERIMENT_DIR}/${PROP_POS}pos${PROP_NEG}neg/forward/${UNET_FW}" --unet_bw_weights_load "${EXPERIMENT_DIR}/${PROP_POS}pos${PROP_NEG}neg/backward/${UNET_BW}" --results_dir $RESULTS_DIR
