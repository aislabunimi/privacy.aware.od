...
if not train_backward_on_disturbed_sets:
...
if train_backward_on_disturbed_sets: #carico i dataloader appositi del dataset disturbato
	disturbed_train_dataloader, disturbed_val_dataloader = load_disturbed_dataset(disturbed_train_img_folder, disturbed_train_ann, disturbed_val_img_folder, disturbed_val_ann, train_img_folder, val_img_folder, train_batch_size, val_batch_size)
else:
	train_dataloader, val_dataloader= load_dataset(train_img_folder, train_ann_file, val_img_folder, val_ann_file, train_batch_size, val_batch_size, save_disturbed_dataset)
...
if(train_only_tasknet and not train_backward_on_disturbed_sets):
	...
	for epoch in range(1, num_epochs+1): #itero ora facendo un train e un test per ogni epoca
    		...
    		log['VAL_LOSS'].append(val_tasknet(val_dataloader, epoch, device, val_loss, tasknet_save_path, tasknet, tasknet_optimizer, tasknet_scheduler, ap_log_path))
